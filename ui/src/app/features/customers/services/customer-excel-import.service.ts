...
import { CustomerDraft, emptyCustomer } from '../models/customer.model';
import {
  CustomerImportParseResult,
  CustomerImportProgress,
} from '../models/customer-import.model';

type ProgressReporter = (progress: CustomerImportProgress) => void;
...
const HEADER_ALIASES: Array<{
  ...
  {
    labels: ['شناسه/كد ملي', 'شناسه/کد ملی', 'کد ملی'],
    field: 'national_id',
  },
  {
    labels: ['كد اقتصادي', 'کد اقتصادی'],
    field: 'economic_code',
  },
  {
    labels: ['كد پستي', 'کد پستی'],
    field: 'postal_code',
  },
  {
    labels: ['آدرس'],
...
export class CustomerExcelImportService {
  ...
  async parse(
    ...
    if (!Array.from(headers.values()).includes('title')) {
      throw new Error('ستون عنوان در ردیف اول فایل پیدا نشد.');
    }

    const validRows: CustomerDraft[] = [];
    const skippedRows: CustomerImportParseResult['skippedRows'] = [];

    for (const rowElement of rowElements.slice(1)) {
      const rowNumber = Number(rowElement.getAttribute('r')) || 0;
      const customer: CustomerDraft = { ...emptyCustomer };
  ...
  private buildHeaderMap(): Map<string, keyof CustomerDraft> {
    const headerMap = new Map<string, keyof CustomerDraft>();
  ...
  ): Map<number, keyof CustomerDraft> {
  ...
  private report(
    ...
    status: CustomerImportProgress['status'],
    label: string,
    detail?: string,
  ): void {
