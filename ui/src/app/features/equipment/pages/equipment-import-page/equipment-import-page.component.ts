...
import { EquipmentImportParseResult, EquipmentImportProgress } from '../../models/equipment-import.model';
import { EquipmentCatalogApiService } from '../../services/equipment-catalog-api.service';
import { EquipmentExcelImportService } from '../../services/equipment-excel-import.service';

@Component({
  selector: 'app-equipment-import-page',
  standalone: true,
  imports: [CommonModule, ButtonModule, CardModule, MessageModule, TableModule],
  templateUrl: './equipment-import-page.component.html',
  styleUrl: './equipment-import-page.component.css',
})
export class EquipmentImportPageComponent {
  selectedFileName = '';
  parseResult: EquipmentImportParseResult | null = null;
  parsing = false;
  importing = false;
  message = '';
  error = '';
  parseStatus = '';
  parseProgress: EquipmentImportProgress[] = [];

  constructor(
    private readonly excelImport: EquipmentExcelImportService,
    private readonly equipmentCatalog: EquipmentCatalogApiService,
  ...
  async handleFileSelected(event: Event): Promise<void> {
    ...
    try {
      ...
      this.message = this.validCount
        ? 'فایل با موفقیت خوانده شد. برای جایگزینی کاتالوگ تجهیزات دکمه ورود را بزنید.'
  ...
  importEquipment(): void {
    ...
    this.equipmentCatalog
      .replaceAll(this.parseResult.validRows)
      .pipe(finalize(() => (this.importing = false)))
      .subscribe({
        next: () => {
          this.message = `${this.validCount} تجهیز با موفقیت وارد کاتالوگ شد.`;
          this.refreshView();
        },
        error: () => {
          this.error =
            'ورود تجهیزات انجام نشد. فایل یا اتصال به دیتابیس را بررسی کنید.';
          this.refreshView();
        },
      });
  }
  ...
  private addProgress(progress: EquipmentImportProgress): void {
